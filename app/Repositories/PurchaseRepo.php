...
class PurchaseRepo{
    ...
    public static function generatePoNumber(){
        $prefix = "P".date("Y.m");
        $data = Purchase::select(DB::raw("max(RIGHT(purchase_number, 5)) as current"))->where(DB::raw("LEFT(purchase_number, 8)") , $prefix)->get()->first();
        $current = 0;
    ...
    public static function findByIdEncrypted($id){
        try {
            $model = Purchase::where(DB::raw("SHA1(id)"), $id)->get()->first();
            $data = null;
            if($model){
                $data = [
                    "id" => sha1($model->id),
